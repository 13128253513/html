...
import com.vcode.ticket.model.UserInfo;
import com.vcode.ticket.utils.*;
...
public class HomePage {
	...
	private void initialize() {
		try {
			...
			userInfo.setSearchTrains(new LinkedList<>());
			userInfo.setSearchPassengerName(new ArrayList<>());
			userInfo.setSearchSeat(new ArrayList<>());
			userInfo.setPassenger(new HashMap<>());
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (TrainInfo.isBrushTicket) {
					if (TrainInfo.isPoolRun) {
						TrainInfo.isPoolRun = false;
						LoggerUtils.printLog("已停止刷票");
						btnNewButton.setText("自动刷票");
					} else {
						TrainInfo.isPoolRun = true;
		...
		chckbxNewCheckBox_1.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (chckbxNewCheckBox_1.isSelected()) {
					btnNewButton.setText("自动刷票");
					TrainInfo.isBrushTicket=true;
				} else {
					if (!TrainInfo.isPoolRun) {
						btnNewButton.setText("手动查票");
						TrainInfo.isBrushTicket=false;
					} else {
						LoggerUtils.printLog("请先停止刷票");
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == e.BUTTON3) {
					...
					for (int row : rows) {
						Object trainsNo=table.getValueAt(row, 0);
						if (!model_train.contains(trainsNo)) {
							model_train.addElement(trainsNo);
							MyThreadLocal.get().getSearchTrains().add(trainsNo.toString().trim());
						}
					}
				}
				if (e.getClickCount() == 2) {
					//new HomeMethods().start();
				}
			}
		});
		...
		JScrollPane scroll = new JScrollPane(LoggerUtils.getTextArea());
		...
		panel_3.add(spinner);
		spinner.setModel(new SpinnerNumberModel(100, 0, 300, 10));
		...
		panel_3.add(scrollPane_2);
						...
						list_1.addMouseListener(new MouseAdapter() {
							...
							public void mouseClicked(MouseEvent e) {
								...
								if (selected > -1) {
									model_train.remove(selected);
									MyThreadLocal.get().getSearchTrains().remove(selected);
								}
							}
						});
									...
									public void mouseClicked(MouseEvent e) {
										if (e.getClickCount() >= 2) {
											if (list_2.getSelectedIndex() > -1) {
												TrainInfo.seatLevel.clear();
												for (int i = 0; i < list_2.getModel().getSize(); i++) {
													if (i != list_2.getSelectedIndex()) {
														TrainInfo.seatLevel.addElement(list_2.getModel().getElementAt(i).toString().trim());
													}
												}
												list_2.setModel(TrainInfo.seatLevel);
											}
										}
									}
								});
											...
											public void mouseClicked(MouseEvent e) {
												if (e.getClickCount() >= 2) {
													...
													if (list_3.getSelectedIndex() > -1) {
														MyThreadLocal.get().getSearchPassengerName().clear();
														for (int i = 0; i < list_3.getModel().getSize(); i++) {
															if (i != list_3.getSelectedIndex()) {
																...
																MyThreadLocal.get().getSearchPassengerName().add(
																		list_3.getModel().getElementAt(i).toString().trim());
															}
														}
		...
		button_2.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				MyThreadLocal.get().getSearchTrains().clear();
			}
		});
	...
	private void checkbrushVotesInfo() {
		...
		if ("".equals(textField.getText())) {
			LoggerUtils.printLog(format.format(new Date()) + "：请输入出发地\r\n");
			return;
		}
		if ("".equals(textField_3.getText())) {
			LoggerUtils.printLog(format.format(new Date()) + "：请输入目的地\r\n");
			return;
		}
		if ("".equals(textField_2.getText())) {
			LoggerUtils.printLog(format.format(new Date()) + "：请输入日期\r\n");
			return;
		}
		if (list_3.getModel().getSize() <= 0) {
			LoggerUtils.printLog(format.format(new Date()) + "：请选择乘车人\r\n");
			return;
		}
		if (list_2.getModel().getSize() <= 0) {
			LoggerUtils.printLog(format.format(new Date()) + "：请先选择席别\r\n");
			return;
		}
		if (TrainInfo.isBrushTicket) {
			if (list_1.getModel().getSize() <= 0) {
				LoggerUtils.printLog(format.format(new Date()) + "：请先切换到手动查票模式下，查询出车次后，并右键添加到车次列表后再切为自动刷票模式\r\n");
				return;
			}
		}

		ListModel<Object> model = list_3.getModel();
		List<String> searchPassenger=new ArrayList<>();
		for (int i = 0; i < model.getSize(); i++) {
			searchPassenger.add(model.getElementAt(i).toString());
		}

		MyThreadLocal.get().setFromStation(map.get(textField.getText())[2]);
		MyThreadLocal.get().setToStation(map.get(textField_3.getText())[2]);
		MyThreadLocal.get().setTrainDate(textField_2.getText());
		MyThreadLocal.get().setBackTrainDate(textField_2.getText());
		packagingseatOther();//获取选中的座位号
		MyThreadLocal.get().setSearchPassengerName(searchPassenger);//选中的乘客

		if (TrainInfo.isPoolRun){
				for (String trainNo : MyThreadLocal.get().getSearchTrains()){
					MyThreadLocal.get().setCurrentThreadTrain(trainNo);
					ThreadPoolUtils.getsInstance().execute(new BrushTicketMethods());//刷票模式,直到买到票结束
				}
        }else {
			window.table.setModel(new DefaultTableModel(new Object[][] {}, new String[] {
					"\u8F66\u6B21", "\u51FA\u53D1\u5730", "\u76EE\u7684\u5730",
					"\u5386\u65F6", "\u53D1\u8F66\u65F6\u95F4",
					"\u5230\u8FBE\u65F6\u95F4", "\u5546\u52A1", "\u7279\u7B49",
					"\u4E00\u7B49", "\u4E8C\u7B49", "\u9AD8\u8F6F", "\u8F6F\u5367",
			...
			window.datalist.clear();
			TrainInfo.trainInfoList.clear();
			Future<?> future= ThreadPoolUtils.getsInstance().submit(new BrushTicketMethods());//手动查询模式
			try {
				future.get();
				for (String[] value : TrainInfo.trainInfoList){
					addRow(value ,TrainInfo.stationCode);
				}
			} catch (Exception e) {
	...
	public int[] packagingseatOther() {
		...
		Integer[] seatOther = new Integer[model2.getSize()];
		if (TrainInfo.isBrushTicket) {
			TrainInfo.seatLevel.clear();
			for (int i = 0; i < model2.getSize(); i++) {
				String seatName=model2.getElementAt(i).toString().trim();
				TrainInfo.seatLevel.addElement(seatName);
				if ("商务座".equals(seatName)) {
					seatOther[i] = 32;
				} else if ("特等座".equals(seatName)) {
					seatOther[i] = 25;
		...
		MyThreadLocal.get().setSearchSeat(Arrays.asList(seatOther));
		return null;
	}
	...
	private DefaultListModel<Object> getPassengerDTOs() {
		...
		VHttpPost post = new VHttpPost(URLUtils.getInitBusPeople().currentURL());
		...
		try {
			...
			if (userListObj.length() < 1) {
				LoggerUtils.printLog(format.format(new Date()) + "：" + new JSONObject(body).get("messages") + "\r\n");
			} else {
				...
				for (int i = 0; i < userArr.length(); i++) {
					...
					MyThreadLocal.get().getPassenger().put(obj.get("passenger_name").toString(),obj);
				}
			}
		} catch (JSONException e) {
			LoggerUtils.printLog(format.format(new Date()) + "：获取乘客列表失败，请联系作者QQ：3094759846\r\n");
		}
		return model_Seats;
	}
